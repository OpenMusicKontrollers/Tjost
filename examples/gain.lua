...
audio_out = {
	plugin('audio_out', 'audio.out.1'),
	plugin('audio_out', 'audio.out.2'),
}
...
audio_in = {
	plugin('audio_in', 'audio.in.1', function(time, path, fmt, last, typ, b)
	...
	plugin('audio_in', 'audio.in.2', function(time, path, fmt, last, typ, b)
		local buf = buf_t(b.raw)
		for i = 0, #b/4-1 do
			buf[i] = buf[i] * gain[2]
		end
